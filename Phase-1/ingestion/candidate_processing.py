...
STAGE_A_PROVENANCE_REASON_BY_FIELD = {
    "source_name": "missing_provenance_source_name",
    "source_uri": "missing_provenance_source_uri",
    "collected_at": "missing_provenance_collected_at",
}
STAGE_A_RISK_REASON_BY_RIGHTS_STATUS = {
    "unknown": "rights_unknown",
    "restricted": "rights_restricted",
}
STAGE_A_INVALID_RIGHTS_REASON = "rights_invalid"
STAGE_A_HAZARD_REASON_CODES = frozenset(
    {"pii_detected", "secret_detected", "benchmark_contamination", "poisoning_suspected"}
)
...
STAGE_A_POLICY_REASON_CODES = {
    "stage_a_provenance_contract": frozenset(
        STAGE_A_PROVENANCE_REASON_BY_FIELD.values()
    ),
    "stage_a_risk_quarantine": frozenset(
        {
            *STAGE_A_HAZARD_REASON_CODES,
            *STAGE_A_RISK_REASON_BY_RIGHTS_STATUS.values(),
            STAGE_A_INVALID_RIGHTS_REASON,
        }
    ),
...
def process_candidate(
    raw: Dict[str, Any], *, index: int = 0, stage_a_policy: str = "provenance_safety_v1"
) -> Dict[str, Any]:
    if stage_a_policy not in {"provenance_safety_v1", "text_only_v2"}:
    ...
    if not text_only:
        for field, value in provenance.items():
            if value.strip().lower() in {"", "unknown", "none", "n/a"}:
                reasons.append(STAGE_A_PROVENANCE_REASON_BY_FIELD[field])
        if rights_status != "allowed":
            reasons.append(STAGE_A_RISK_REASON_BY_RIGHTS_STATUS.get(rights_status, STAGE_A_INVALID_RIGHTS_REASON))
        for name in STAGE_A_HAZARD_REASON_CODES:
            if hazards[name]:
                reasons.append(name)
    reasons.extend(_validity_reasons(raw_text, normalized["text"], raw))
