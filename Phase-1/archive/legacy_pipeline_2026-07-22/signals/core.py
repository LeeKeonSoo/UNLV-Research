...
def style_bucket_from_text(text: str) -> str:
    lowered = text.lower()
    lines = [line.strip() for line in text.splitlines() if line.strip()]
...
class CoreMetricScorer:
    def __init__(
        ...
        simhash_prefix_counts: Dict[str, int] | None = None,
        dataset_cluster_size_rarity: Dict[str, Dict[int, float]] | None = None,
        ...
        self._simhash_prefix_counts = (
            dict(simhash_prefix_counts) if simhash_prefix_counts is not None else self._load_simhash_prefix_counts()
        )
        ...
        self._dataset_cluster_size_rarity = (
            dict(dataset_cluster_size_rarity)
            if dataset_cluster_size_rarity is not None
            else self._load_dataset_cluster_size_rarity()
        )
        ...
        details.update(
            {
                ...
                "canonical_core_axis": "Selection Value Evidence",
                "legacy_core_axis_alias": "Quality",
                "construct_claim": "observable_pre_outcome_selection_evidence_not_intrinsic_quality",
                "hard_reject_authority": False,
    ...
    def structural_validity_score(self, text: str) -> Dict[str, Any]:
        ...
        style_bucket = style_bucket_from_text(text)
    ...
    def shingle_near_duplicate_risk_score(
        ...
        style_bucket = style_bucket_from_text(text or "") if text is not None else "general_prose"
        repeat_divisor = 0.55
    ...
    def split_metric_groups(self, metrics: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    ...
    def score_chunk_grouped(self, chunk_meta: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        return self.split_metric_groups(self.score_chunk(chunk_meta))
    ...
    def score_chunks_grouped(self, chunk_metas: List[Dict[str, Any]]) -> List[Dict[str, Dict[str, Any]]]:
        return [self.split_metric_groups(metrics) for metrics in self.score_chunks(chunk_metas)]
