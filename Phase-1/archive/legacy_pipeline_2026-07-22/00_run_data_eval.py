...
DEFAULT_FINEWEB_EDU_LIMIT = 500000
DEFAULT_FINEWEB_EDU_TARGET_TOKENS = 250_000_000
DEFAULT_FINEWEB_EDU_CONFIG = "sample-10BT"
CORE_STEPS = (
    ("validation/test_decision_contracts.py", None),
    ("validation/test_stage0_processing.py", None),
    ("validation/test_style_taxonomy_contract.py", None),
    ("30_process_stage0_candidates.py", None),
    ("29_validate_stage0_contract.py", None),
    ...
    ("15_run_selector_baseline_audit.py", None),
    ("21_build_utility_transfer_gap_report.py", None),
    ("23_build_core_proxy_alignment_report.py", None),
    ("24_build_core_proxy_calibration_report.py", None),
    ("20_build_curation_readiness_report.py", None),
    ("25_build_stage_c_protocol_decision_report.py", None),
    ("26_build_strict_baseline_control_report.py", None),
    ("27_build_curation_decision_report.py", None),
    ("28_build_paper_evidence_table.py", None),
    ("06_validate_outputs.py", "report"),
)
EXTENDED_STEPS = (
    ("07_run_property_benchmarks.py", None),
    ("31_build_openwebtext2_slice_diagnostic.py", None),
...
def _should_refresh_fineweb_edu_sample(
    source: Path,
    *,
    limit: int,
    target_tokens: int,
    ...
    try:
        ...
        return True, "manifest unreadable"
    if str(manifest.get("hf_config") or "") != str(hf_config):
        return True, "hf config changed"
    if str(manifest.get("tokenizer_name") or "") != str(tokenizer_name):
        return True, "tokenizer changed"
    recorded_target = int(manifest.get("target_tokens") or 0)
    if recorded_target != int(target_tokens):
        return True, "target tokens changed"
    approx_tokens = int(manifest.get("approx_tokens_written") or 0)
    recorded_limit = int(manifest.get("requested_limit") or manifest.get("records_written") or 0)
    if approx_tokens < int(target_tokens * 0.95) and recorded_limit < int(limit):
        return True, "existing sample capped below requested token target"
    return False, "up to date"


def _maybe_prepare_fineweb_edu_sample(
    selected_specs,
    *,
    limit: int,
    target_tokens: int,
    ...
    for raw in selected_specs:
        ...
        name = str(raw.get("name") or "").strip()
        source = _resolve_source_path(str(raw.get("source") or ""))
        if name != "fineweb_edu_sample":
            continue
        refresh, reason = _should_refresh_fineweb_edu_sample(
            source,
            limit=int(limit),
        ...
        if refresh:
            helper = PROJECT_DIR / "prepare_fineweb_edu_sample.py"
            cmd = [
                sys.executable,
                str(helper.name),
                "--limit",
            ...
            print("\n[data-eval] auto-prepare FineWeb-Edu sample")
            print(f"  reason: {reason}")
            print(f"  target_tokens: {target_tokens} ({tokenizer_name})")
            print("  $", " ".join(cmd))
            result = subprocess.run(cmd, cwd=PROJECT_DIR)
            if result.returncode != 0:
                raise SystemExit(result.returncode)
...
def main() -> int:
    ...
    parser.add_argument("--fineweb-edu-limit", type=int, default=DEFAULT_FINEWEB_EDU_LIMIT)
    parser.add_argument("--fineweb-edu-target-tokens", type=int, default=DEFAULT_FINEWEB_EDU_TARGET_TOKENS)
    parser.add_argument("--fineweb-edu-config", default=DEFAULT_FINEWEB_EDU_CONFIG)
    ...
    _maybe_prepare_fineweb_edu_sample(
        selected_specs,
        limit=args.fineweb_edu_limit,
        target_tokens=args.fineweb_edu_target_tokens,
        hf_config=args.fineweb_edu_config,
