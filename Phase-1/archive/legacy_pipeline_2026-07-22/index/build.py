...
CLUSTER_ASSIGN_BATCH_SIZE = 4096
SQL_UPDATE_BATCH_SIZE = 5000
...
def _cluster_assignments(vectorizer: HashingVectorizer, kmeans: MiniBatchKMeans, chunks: List[Dict[str, Any]]) -> List[tuple]:
    if not chunks:
        return []
    matrix = vectorizer.transform([chunk["text"] for chunk in chunks])
    cluster_ids = kmeans.predict(matrix)
    return [(int(cluster_id), chunk["chunk_uid"]) for cluster_id, chunk in zip(cluster_ids, chunks)]
...
def build_index(dataset_config: Path) -> Dict[str, Any]:
    ...
    def _flush_assignments() -> None:
        assignment_rows = _cluster_assignments(vectorizer, kmeans, assignment_batch)
        for cluster_id, chunk_uid in assignment_rows:
            cluster_sizes[cluster_id] += 1
            update_rows.append((cluster_id, chunk_uid))
            if len(update_rows) >= SQL_UPDATE_BATCH_SIZE:
                _flush_updates()
        assignment_batch.clear()

    for chunk in tqdm(iter_chunks(specs), total=total_chunks, desc="[02] index pass 2", unit="chunk"):
        assignment_batch.append(chunk)
        if len(assignment_batch) >= CLUSTER_ASSIGN_BATCH_SIZE:
            _flush_assignments()
    if assignment_batch:
        _flush_assignments()
    if update_rows:
