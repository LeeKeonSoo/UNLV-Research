#!/usr/bin/env python3
"""Build the frozen code-domain v2 confirmatory decision report."""
...
DEFAULT_PROTOCOL = Path("configs") / "code_domain_v2_confirmatory_protocol_qwen3_4b.json"
DEFAULT_PROTOCOL_REPORT = OUTPUT_DIR / "validation" / "code_domain_v2_confirmatory_protocol_qwen3_4b_report.json"
DEFAULT_OUTPUT_DIR = OUTPUT_DIR / "code_domain_v2_confirmatory_qwen3_4b"
DEFAULT_OUTPUT = OUTPUT_DIR / "validation" / "code_domain_v2_confirmatory_decision_report.json"
DEFAULT_EVALPLUS_REPORT = OUTPUT_DIR / "validation" / "code_domain_v2_evalplus_confirmatory_guardrail_report.json"
DEFAULT_GENERAL_TEXT_REPORT = OUTPUT_DIR / "validation" / "code_domain_v2_general_text_confirmatory_guardrail_report.json"
DEFAULT_GENERAL_TASK_REPORT = OUTPUT_DIR / "validation" / "code_domain_v2_general_task_confirmatory_guardrail_report.json"
...
def _trained_arms(protocol: Dict[str, Any]) -> List[str]:
    return [str(arm) for arm in protocol["training_arms"] if str(arm) != "base_no_update"]
...
def _load_guardrail_report(path: Path, expected_pass_status: str) -> Dict[str, Any]:
    ...
    evidence_state = "passed" if status == expected_pass_status else "incomplete" if status.endswith("_incomplete") else "failed"
    return {
        "status": status,
        "path": str(path),
        "passed": status == expected_pass_status,
...
def _guardrail_missing_or_incomplete(guardrail: Dict[str, Any]) -> bool:
    return str(guardrail.get("evidence_state") or "") in {"missing", "incomplete"}


def _guardrail_failed(guardrail: Dict[str, Any]) -> bool:
    return str(guardrail.get("evidence_state") or "") == "failed"
...
def build(
    ...
    trained_arms = _trained_arms(protocol)
    rule = protocol["primary_success_rule"]
    margin = float(rule["required_absolute_nll_reduction"])
    blockers: List[str] = []
    ...
    if blockers:
        nll_gate: Dict[str, Any] = {"status": "incomplete", "blockers": blockers}
        overall_status = "v2_confirmatory_decision_incomplete"
    else:
        ...
        curated = eval_results[treatment]
        stage_a = eval_results[primary_baseline]
        raw = eval_results["raw_random_equal_budget"]
        reference = eval_results[reference_arm]
        ...
        stage_a_delta = _paired_delta(stage_a, curated, label="Stage-A-random minus curated")
        raw_delta = _paired_delta(raw, curated, label="raw-random minus curated")
        reference_delta = _paired_delta(reference, curated, label="known-high-quality minus curated")
        primary_margin_pass = primary_reduction >= margin
        paired_seed_pass = stage_a_delta["all_seed_deltas_positive"]
        raw_direction_pass = raw_direction_reduction > 0
        nll_gate = {
            "status": "passed" if primary_margin_pass and paired_seed_pass and raw_direction_pass else "failed",
            "primary_margin_required_absolute_nll_reduction": margin,
            "curated_vs_stageA_random_mean_nll_reduction": primary_reduction,
            "curated_vs_stageA_random_margin_pass": primary_margin_pass,
            "curated_vs_stageA_random_all_paired_seed_pass": paired_seed_pass,
            ...
            "paired_deltas": {
                "stageA_random_minus_curated": stage_a_delta,
                "raw_random_minus_curated": raw_delta,
                "known_high_quality_minus_curated": reference_delta,
            },
        }
        guardrail_issues = []
        if evalplus_guardrail["status"] == "missing":
            guardrail_issues.append("EvalPlus confirmatory guardrail evidence is missing")
        elif _guardrail_missing_or_incomplete(evalplus_guardrail):
            guardrail_issues.append(
                f"EvalPlus confirmatory guardrail evidence is incomplete: {evalplus_guardrail['status']}"
            )
        ...
        elif _guardrail_missing_or_incomplete(general_text_guardrail):
            guardrail_issues.append(
                f"general-text NLL confirmatory retention evidence is incomplete: {general_text_guardrail['status']}"
            )
        elif not general_text_guardrail["passed"]:
            guardrail_issues.append(
                f"general-text NLL confirmatory retention guardrail is not passing: {general_text_guardrail['status']}"
            )
        if general_task_guardrail["status"] == "missing":
            guardrail_issues.append("general-task confirmatory retention evidence is missing")
        elif _guardrail_missing_or_incomplete(general_task_guardrail):
            guardrail_issues.append(
                f"general-task confirmatory retention evidence is incomplete: {general_task_guardrail['status']}"
            )
        ...
        if not primary_margin_pass:
            overall_status = "v2_confirmatory_decision_reject_primary_margin_failure"
        elif not paired_seed_pass:
            overall_status = "v2_confirmatory_decision_reject_paired_seed_failure"
        elif not raw_direction_pass:
            overall_status = "v2_confirmatory_decision_reject_raw_direction_failure"
        elif any(_guardrail_failed(guardrail) for guardrail in (evalplus_guardrail, general_text_guardrail, general_task_guardrail)):
            overall_status = "v2_confirmatory_decision_reject_guardrail_failure"
        elif guardrail_issues:
            overall_status = "v2_confirmatory_decision_abstain_missing_required_guardrails"
        else:
            overall_status = "v2_confirmatory_decision_passed"

    report = {
        "schema_version": "code-domain-v2-confirmatory-decision-report-v1",
        ...
        "summary": {
            ...
            "stage_c_guardrails": {
                "evalplus_confirmatory": {
                    ...
                    "evidence_state": evalplus_guardrail["evidence_state"],
                ...
                "general_text_nll_retention": {
                    ...
                    "evidence_state": general_text_guardrail["evidence_state"],
                ...
                "general_task_retention": {
                    ...
                    "evidence_state": general_task_guardrail["evidence_state"],
        ...
        "claim_boundary": (
            "V2 confirmatory decision report. It may read frozen confirmatory outcomes, "
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build code-domain v2 confirmatory decision report.")
    ...
    return 0 if report["status"] != "v2_confirmatory_decision_incomplete" else 2
