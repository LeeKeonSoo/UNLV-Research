#!/usr/bin/env python3
"""Freeze no-outcome development executable-evaluation candidates."""
...
DEFAULT_CONTRACT = Path("configs") / "temporal_code_development_executable_expansion_v1.json"
...
DEFAULT_OUTPUT = OUTPUT_DIR / "temporal_code_collection" / "temporal_code_development_execution_expansion_plan.json"
...
def freeze(
    ...
    if len(selected) < int(contract["minimum_repository_count"]):
        blockers.append("insufficient_untouched_development_code_and_test_candidates")
    status = "frozen_before_tranche_content_fetch" if not blockers else contract["failure_decision"]
    report = {
        "schema_version": "temporal-code-development-execution-expansion-plan-v1",
        "status": status,
        "contract": contract,
        "source_sha256": {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Freeze development executable expansion.")
