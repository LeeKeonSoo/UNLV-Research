...
def _arm_records(
    *,
    ...
    if arm == "positive_control":
        ...
        return records[:limit]
    if arm in {"negative_control", "low_quality_negative_control", "corrupted_negative_control"}:
        records = sorted(
            stage_a,
            key=lambda record: (
                _quality(record),
...
def _corrupt_text_for_negative_control(text: str, *, uid: str, seed: int) -> str:
...
def _maybe_transform_arm_pairs(arm: str, pairs: List[tuple[str, str]], *, seed: int) -> List[tuple[str, str]]:
    if arm != "corrupted_negative_control":
        return pairs
    return [
        (f"{uid}::corrupted", _corrupt_text_for_negative_control(text, uid=uid, seed=seed))
...
def _score_arm(
    *,
    ...
    pairs = _maybe_transform_arm_pairs(str(arm), pairs, seed=int(args.seed))
...
def _probe_sensitivity_order(arm_results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    positive = float((by_arm.get("positive_control") or {}).get("delta_nll") or 0.0)
    random = float((by_arm.get("stageA_random") or {}).get("delta_nll") or 0.0)
    negative_arm = "corrupted_negative_control" if "corrupted_negative_control" in by_arm else "negative_control"
    negative = float((by_arm.get(negative_arm) or {}).get("delta_nll") or 0.0)
    selected = float((by_arm.get("selected") or {}).get("delta_nll") or 0.0)
    return {
        "expected_order": f"positive_control > stageA_random > {negative_arm}",
        "canonical_negative_control": negative_arm,
        "probe_valid": bool(positive > random > negative),
        "positive_gt_random": bool(positive > random),
        "random_gt_negative": bool(random > negative),
        "selected_gt_random": bool(selected > random),
        "order_pass": bool(positive > random > negative),
        "delta_nll_by_arm": {
            arm: result.get("delta_nll") for arm, result in sorted(by_arm.items())
        },
        "interpretation": (
            "If probe_valid/order_pass is false, do not treat Utility as selector evidence before revisiting the probe/protocol. "
            "If probe_valid is true but selected_gt_random is false, the probe is sensitive and the current selector is the likely bottleneck."
        ),
    }


def _root_cause_decision(probe_sensitivity: Dict[str, Any], current_failure: Dict[str, Any]) -> Dict[str, Any]:
    order_pass = bool(probe_sensitivity.get("order_pass"))
    ...
    if not order_pass:
        ...
        action = "Do not tune the selector further until the Utility probe separates positive/random/negative controls with enough budget."
    elif in_domain_fails and not selected_gt_random:
        primary = "selector_learnability_bottleneck"
        action = "The probe is sensitive and the selected subset does not beat random; focus on selector policy or subset construction."
    ...
    return {
        ...
        "selector_tuning_allowed": bool(order_pass),
        ...
        "reasoning": {
            "probe_order_pass": order_pass,
            "utility_probe_valid": order_pass,
...
def _status_from_audit(dataset: str, audit_payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not isinstance(root, dict):
        root = {}
    probe_valid = sensitivity.get("probe_valid", sensitivity.get("order_pass"))
    return {
        "available": True,
        "probe_valid": bool(probe_valid),
        "status": "valid" if bool(probe_valid) else "invalid",
        "selector_tuning_allowed": bool(root.get("selector_tuning_allowed", bool(probe_valid))),
...
def main() -> int:
    ...
    parser.add_argument(
        "--arms",
        nargs="*",
        default=["selected", "positive_control", "stageA_random", "corrupted_negative_control", "low_quality_negative_control"],
    )
    ...
    try:
        for dataset in datasets:
            ...
            output["datasets"][str(dataset)] = {
                ...
                "selector_tuning_allowed": bool(probe_sensitivity.get("order_pass")),
            }
