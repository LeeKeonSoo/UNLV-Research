...
def test_stage_b_proposes_and_stage_c_may_veto_without_erasing_trace() -> None:
    proposals = propose_quality_removals(
    ...
    assert final["remove-me"].final_action == "remove"
    assert final["keep-me"].final_action == "retain"
    assert final["keep-me"].stage_b_reason_code == "quality_normal_unanimous_fail"
    assert final["keep-me"].stage_c_reason_code == "coverage_veto_retain"


if __name__ == "__main__":
    test_stage_b_proposes_and_stage_c_may_veto_without_erasing_trace()
    print("[quality-stage-bridge-v1] Stage B proposal and Stage C Coverage veto: pass")
