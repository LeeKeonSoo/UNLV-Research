...
def test_teacher_panel_is_the_only_current_model_quality_authority() -> None:
    ...
    assert registry_references["quality_teacher_panel"] == "configs/quality_teacher_panel_v2.json"
    ...
    assert "quality.teacher_panel_v2" in policy_ids
    assert "quality.teacher_panel_v2" in provider_ids
    ...
    assert "quality.teacher_panel_v2" in profiled_policy_ids
    assert "quality-teacher-panel-v2" in registered_provider_ids
    assert all("contrastive" not in item for item in profiled_policy_ids | registered_provider_ids)
    assert "configs/quality_teacher_panel_v2.json" in POLICY_FINGERPRINT_CONFIGS
...
if __name__ == "__main__":
    test_teacher_panel_is_the_only_current_model_quality_authority()
