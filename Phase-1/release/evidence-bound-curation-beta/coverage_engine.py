...
def evaluate_coverage(request: CoverageRequest, provider: ProviderManifest) -> CoverageDecision:
    ...
    facility_index = FacilityLocationIndex.build(
        eligible_order,
    ...
    restoration_traces: list[CoverageRestorationTrace] = []
    ...
    for family in sorted(request.redundancy_families, key=lambda item: item.family_id):
        ...
        if preferred is not None and preferred in choice_pool:
            ...
            representative, gain = facility_index.choose(
                choice_pool,
                frozenset(working),
            )
        ...
        if not surviving_candidates:
            ...
            restoration_traces.append(
                CoverageRestorationTrace(
                    chunk_uid=representative,
                    group_id=family.family_id,
                    view=CoverageView.REDUNDANCY_FAMILY,
                    evidence_artifact_sha256=family.evidence_artifact_sha256,
    ...
    for stratum in sorted(request.strata, key=lambda item: item.stratum_id):
        ...
        if not choice_pool:
            return _abstain(request, "coverage_restoration_candidate_ceiling_exhausted")
        representative, _ = facility_index.choose(
            choice_pool,
            frozenset(working),
        )
        working.add(representative)
        ...
        restoration_traces.append(
            CoverageRestorationTrace(
                ...
                view=stratum.view,
                evidence_artifact_sha256=stratum.evidence_artifact_sha256,
                selection_method="facility_location_marginal_gain_then_uid",
            )
        )
    ...
    return CoverageDecision(
        ...
        nearest_representative_radius=facility_index.nearest_radius(final_survivors),
        ...
        restoration_traces=tuple(
            sorted(restoration_traces, key=lambda trace: trace.chunk_uid)
        ),
    )
