...
from quality_model_evidence import QualityPolicyEvidence
...
def score_quality_rows_distilled(
    ...
    *,
...
) -> tuple[dict[str, tuple[QualityPolicyEvidence, ...]], dict]:
    ...
    results: dict[str, tuple[QualityPolicyEvidence, ...]] = {}
    uncertain_units = 0
    for row_index, uid in enumerate(row_uids):
        policy_results: list[QualityPolicyEvidence] = []
        for head in artifact.heads:
            contract = DistilledPolicyContract(
                policy_id=head.policy_id,
                class_labels=head.class_labels,
                failure_threshold=head.failure_threshold,
