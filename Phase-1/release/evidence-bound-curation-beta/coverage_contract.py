...
@dataclass(frozen=True, slots=True)
class CoverageRestorationTrace:
    chunk_uid: str
    group_id: str
    view: CoverageView
    evidence_artifact_sha256: str
    ...
    def __post_init__(self) -> None:
        if not self.chunk_uid or not self.group_id or not self.selection_method:
            raise CoverageContractError("Coverage restoration traces require typed identifiers")
        if not SHA256_RE.fullmatch(self.evidence_artifact_sha256):
            raise CoverageContractError("Coverage restoration traces require frozen evidence")
...
class CoverageDecision:
    ...
    restoration_traces: tuple[CoverageRestorationTrace, ...] = ()
