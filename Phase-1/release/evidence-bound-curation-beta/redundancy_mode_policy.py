...
from redundancy_v2_retrieval import CandidatePair, retrieve_candidate_pairs
...
def _candidate_pairs(
    ...
    if exhaustive:
        pairs = {
            tuple(sorted((left.uid, right.uid))): {"exhaustive_validation"}
            for index, left in enumerate(units)
            for right in units[index + 1 :]
        }
    else:
        pairs = {
            (pair.left_uid, pair.right_uid): set(pair.retrieval_reasons)
            for pair in retrieve_candidate_pairs(units, settings)
        }
    for pair in declared:
        pairs.setdefault(pair, set()).add("declared_equivalence_witness")
    return tuple(
        CandidatePair(left, right, tuple(sorted(reasons)))
        for (left, right), reasons in sorted(pairs.items())
    )
...
def build_redundancy_plan(
    ...
    *,
    ...
    candidate_pairs = _candidate_pairs(units, settings, declared, exhaustive)
    if any(
        pair.left_uid not in by_uid or pair.right_uid not in by_uid
    ...
    decisions = tuple(
        ...
        for left, right in (
            (pair.left_uid, pair.right_uid) for pair in candidate_pairs
        )
    )
