...
def _stage_a_chunk_reasons(row: JsonMap) -> list[str]:
    reasons = row.get("stage_a_hard_gate_reasons")
    return [str(reason) for reason in reasons] if isinstance(reasons, list) else []
...
def build_reason_code_impact_audit(
    ...
    stage_a_rejected_chunks: Iterable[JsonMap],
    ...
    report = {
        ...
        "stages": {
            ...
            "stage_a_chunk_rejection": _stage_impact(
                stage_a_rejected_chunks, _stage_a_chunk_reasons
            ),
