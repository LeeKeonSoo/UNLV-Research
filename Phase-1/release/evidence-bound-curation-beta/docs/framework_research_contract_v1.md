...
| --- | --- | --- |
| `retain_local_positive_support` | the local ranker has Q2+Q3+Q4 pass and no qualified fail | retain with trace |
| `retain_teacher_positive_support` | Luna fallback has Q2+Q3+Q4 pass and no confirmed fail | retain with trace |
| `not_select_qualified_local_fail` | a local Policy has sufficient negative evidence | do not select, with trace |
| `not_select_teacher_confirmed_fail` | Luna repeats a fail decision and reason under a blinded second pass | do not select, with trace |
| `not_select_teacher_no_positive_support` | the completed fallback has no confirmed fail but lacks conjunctive Q2+Q3+Q4 support | do not select, with trace |
| `coverage_veto_retain` | Stage C detects support extinction | explicitly restore and recheck |

An unsupported local decision is not itself a membership decision. It creates
a hash-bound Luna request and stops the run until the observation exists.
Transport or schema failure therefore cannot be converted into either keep or
drop evidence.
...
reference-pool contract invalidates prior calibration. Until recalibration and
the required fallback evidence pass, the affected Policy cannot materialize a
membership decision.
