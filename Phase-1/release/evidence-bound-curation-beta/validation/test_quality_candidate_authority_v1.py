...
def test_distilled_ranker_is_the_only_runtime_quality_provider() -> None:
    manifest = _read_json("configs/curation_framework_v1.json")
    registry = _read_json("configs/runtime_policy_registry_v1.json")
    profiles = _read_json("configs/runtime_policy_profiles_v1.json")
    providers = _read_json("configs/model_provider_registry_v1.json")

    registry_references = manifest["registry_references"]
    assert "quality_teacher_oracle" not in registry_references
    ...
    policy_ids = {policy["id"] for policy in registry["policies"]}
    assert "stage_b_quality_distilled_ranker_v1" in policy_ids
    assert all("teacher" not in item and "contrastive" not in item for item in policy_ids)

    profiled_policy_ids = {
        policy_id
        for profile in profiles["profiles"]
        for policy_id in profile["enabled_policy_ids"]
    }
    registered_provider_ids = {provider["provider_id"] for provider in providers["providers"]}
    assert "stage_b_quality_distilled_ranker_v1" in profiled_policy_ids
    assert "quality-distilled-ranker-v1" in registered_provider_ids
    assert "quality-teacher-luna-single-v1" not in registered_provider_ids
    assert all("contrastive" not in item for item in profiled_policy_ids | registered_provider_ids)
    assert "configs/quality_teacher_luna_single_v1.json" not in POLICY_FINGERPRINT_CONFIGS
...
if __name__ == "__main__":
    test_distilled_ranker_is_the_only_runtime_quality_provider()
    test_contrastive_candidate_files_are_outside_the_active_surface()
    print("[quality-candidate-authority-v1] frozen ranker is the sole runtime Quality provider: pass")
