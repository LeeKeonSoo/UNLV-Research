...
def test_stage_c_restores_only_an_extinct_support_representative() -> None:
    ...
    assert audit["restoration_trace_complete"] is True
    assert audit["restoration_traces"] == [
        {
            "chunk_uid": "b",
            "group_id": "redundancy-family-a-b",
            "view": "redundancy_family",
            "evidence_artifact_sha256": "4" * 64,
            "selection_method": "facility_location_marginal_gain_then_uid",
        }
    ]
    restored_row = next(row for row in final if row["chunk_uid"] == "b")
    retained_row = next(row for row in final if row["chunk_uid"] == "c")
    assert restored_row["stage_c_coverage"]["restoration_trace"] == {
        "group_id": "redundancy-family-a-b",
        "view": "redundancy_family",
        "evidence_artifact_sha256": "4" * 64,
        "selection_method": "facility_location_marginal_gain_then_uid",
    }
    assert "restoration_trace" not in retained_row["stage_c_coverage"]
...
def test_semantic_artifact_preflight_preserves_unicode_line_separator() -> None:
    text = "First mathematical statement.\u2028Second mathematical statement."
    rows = ({"chunk_uid": "a", "text": text, "token_proxy": 5},)
    provider = _provider()
    with tempfile.TemporaryDirectory() as directory:
        root = Path(directory)
        corpus = root / "corpus.jsonl"
        corpus.write_text(
            json.dumps({"uid": "a", "text": text}, ensure_ascii=False) + "\n",
            encoding="utf-8",
        )
        graph = root / "graph.json"
        graph.write_text(
        ...
        audit = validate_semantic_coverage_artifacts(
            universe=rows,
            corpus_path=corpus,
            graph_path=graph,
            provider=provider,
    ...
    assert audit["status"] == "semantic_coverage_artifacts_ready"
    assert audit["universe_chunks"] == 1
...
if __name__ == "__main__":
    ...
    test_semantic_artifact_preflight_preserves_unicode_line_separator()
