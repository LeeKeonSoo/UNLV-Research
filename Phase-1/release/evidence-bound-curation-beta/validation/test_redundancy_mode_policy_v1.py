...
def test_transitive_family_uses_one_stable_id_and_representative_trace() -> None:
    ...
    assert all(removal.representative_uid in plan.proposed_survivor_uids for removal in plan.removals)


def test_retrieval_reasons_survive_into_the_plan_audit_surface() -> None:
    payload = " ".join(f"stable{index}" for index in range(90))
    units = (
        RedundancyUnit("a", payload),
        RedundancyUnit("b", payload.replace(" ", "\n", 12)),
    )

    plan = build_redundancy_plan(units, RedundancySettings(), RedundancyMode.FRAMEWORK)

    assert len(plan.candidate_pairs) == 1
    assert "character_minhash_lsh" in plan.candidate_pairs[0].retrieval_reasons


def test_character_minhash_candidate_cannot_remove_without_equivalence_witness() -> None:
    common = " ".join(f"stable{index}" for index in range(100))
    units = (
        RedundancyUnit("a", f"{common} return 10"),
        RedundancyUnit("b", f"{common} return 11"),
    )

    plan = build_redundancy_plan(
        units, RedundancySettings(), RedundancyMode.FRAMEWORK
    )

    assert "character_minhash_lsh" in plan.candidate_pairs[0].retrieval_reasons
    assert plan.removals == ()
    assert plan.authority_decisions[0].authority_reason_code == (
        "redundancy_substantive_difference_protected"
    )
...
def test_mode_contract_is_runtime_active_and_has_no_external_selector_inputs() -> None:
    ...
    assert contract["operating_points"]["framework"]["uncertain_action"] == "retain"
    assert contract["operating_points"]["framework"]["coverage_veto_required"] is True
...
if __name__ == "__main__":
    ...
    test_retrieval_reasons_survive_into_the_plan_audit_surface()
    test_character_minhash_candidate_cannot_remove_without_equivalence_witness()
