...
def test_manifest_roots_the_frozen_research_contract() -> None:
    ...
    assert manifest["activation"] == "beta_release_v1"
...
def test_manifest_declares_the_complete_core_and_stage_boundary() -> None:
    ...
    assert profile["public_profiles"] == ["framework"]
    assert profile["retained_set_invariant"] == "single_profile"
    assert profile["shared_policy_families_required"] is False
    assert profile["independent_operating_point_calibration_required"] is False
    ...
    assert manifest["registry_references"]["runtime_policy_registry"] == "configs/runtime_policy_registry_v1.json"
    assert manifest["registry_references"]["runtime_policy_profiles"] == "configs/runtime_policy_profiles_v1.json"
    assert "quality_teacher_oracle" not in manifest["registry_references"]
...
def test_threshold_provenance_is_non_optional() -> None:
    ...
    assert manifest["threshold_provenance"]["missing_field_action"] == "retain"
