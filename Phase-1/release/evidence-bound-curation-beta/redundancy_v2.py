...
class RedundancySettings:
    ...
    character_ngram_size: int = 24
    character_minhash_bands: int = 20
    character_minhashes_per_band: int = 13
    ...
    def __post_init__(self) -> None:
        ...
        if self.character_ngram_size < 20:
            raise RedundancyContractError("character_ngram_size must be at least 20")
        if self.character_minhash_bands < 1 or self.character_minhashes_per_band < 1:
            raise RedundancyContractError("Character MinHash dimensions must be positive")
