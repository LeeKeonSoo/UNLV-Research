# ⚠️ IMPORTANT: UPDATE THIS DOCUMENT WHEN CHANGES OCCUR
**이 문서는 프로젝트의 현재 상황을 나타냅니다. 코드 수정, 파일 추가/삭제, 전략 변경 등이 발생하면 반드시 이 문서를 업데이트하세요.**

---

# Phase-1 Project Status - Current Situation

**Last Updated**: February 10, 2026 (Updated: Full pipeline rewrite — all 5 metrics)
...
### ✅ RESOLVED: Khan Academy Data Source

**Solution**: Using `HuggingFaceTB/cosmopedia` khanacademy subset

**Previously Tried (Failed)**:

1. ❌ Khan Academy API - `410 Gone` (deprecated as of ~2023)
2. ❌ HuggingFace `datasets` library in remote env - `403 Forbidden` (proxy)
3. ❌ Direct web scraping - React SPA, blocked in remote env

**Current Solution (✅ Working)**:

- Source: `HuggingFaceTB/cosmopedia` khanacademy config (24,123 entries)
- Dual download strategy: `datasets` library → direct parquet API fallback
...
### 🟡 REMAINING: Run Full 5-Metric Pipeline

- [x] `python 1_extract_khan_taxonomy.py` ✅ (100 course-level prototypes)
- [x] `python 2a_build_corpus_index.py` 🔄 RUNNING (MinHash LSH + TF-IDF index)
- [ ] `python 2_compute_metrics.py` (all 5 metrics; max_batches=5 for test, None for full)
- [ ] `python 3_build_dashboard.py` (4-tab interactive dashboard)
...
1. **Domain Coverage** (현재 구현됨) - Multi-label classification using concept prototypes
2. **Quality** (현재 구현됨) - Educational markers detection
3. **Difficulty** (보류) - Reading level analysis
4. **Redundancy** (보류) - Content deduplication metrics
...
```
Phase-1/
├── collect_khan_academy.py          # Step 0: Data collection (BLOCKED)
├── collect_tinytextbooks.py         # Step 0: Tiny-Textbooks collection (WORKS)
├── 1_extract_khan_taxonomy.py       # Step 1: Build concept prototypes (TF-IDF)
├── 2_compute_metrics.py             # Step 2: Domain + Quality analysis
├── 3_build_dashboard.py             # Step 3: Interactive HTML visualization
├── config.py                        # Configuration constants
├── utils.py                         # Shared utility functions
├── README.md                        # Full pipeline documentation
├── notion_research_log.md           # Research journal (Feb 10, 2026)
...
```
legacy/
├── 1_extract_khan_taxonomy_sbert.py   # SentenceTransformer version (requires network)
├── 2_compute_metrics_sbert.py         # Full version with GPT-2 perplexity
├── SETUP.md                           # Remote execution troubleshooting guide
├── download_khan_data.py              # Failed HuggingFace download attempt
└── [previous analysis files]          # Old graph-based experiments
```
...
**Status**: 🔴 **BLOCKED** (Khan Academy download fails)

**Files**:
- `collect_khan_academy.py` - ❌ Cannot download
- `collect_tinytextbooks.py` - ✅ Works (requires HuggingFace datasets library)
...
**Status**: 🟡 **READY** (waiting for Khan data)
...
**Status**: 🟡 **READY** (waiting for Step 1 output)
...
- ❌ Perplexity (would require GPT-2 download)
- ❌ Reading difficulty (deferred to future work)

---

#### Step 3: Build Dashboard
**Status**: 🟡 **READY** (waiting for Step 2 output)
...
**Current Status**: ❌ BLOCKED - Cannot download data
