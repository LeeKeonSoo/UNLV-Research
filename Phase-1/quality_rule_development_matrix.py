...
def _coverage(not_selected: list[JsonMap], transformations: list[JsonMap], minimum_residual_chars: int, curated: list[JsonMap]) -> JsonMap:
    explicit_reasons = {
    ...
    residual_valid = all(len(str(row.get("text") or "").strip()) >= minimum_residual_chars for row in curated if row.get("chunk_uid") in {item["chunk_uid"] for item in transformations})
    return {
...
def _arm(
    ...
    return {
        ...
        "coverage": _coverage(not_selected, transformations, minimum_residual_chars, curated),
...
def run_quality_matrix(rows: Iterable[JsonMap], *, minimum_residual_chars: int, token_counter: TokenCounter) -> JsonMap:
    ...
    arms = {
        "baseline": _arm("baseline", source_rows, source_rows, [], {}, minimum_residual_chars, token_counter, False),
        **{
            arm_id: _arm(arm_id, source_rows, source_rows, [], rules, minimum_residual_chars, token_counter, False)
        ...
        **{
        ...
        "all_active_quality": _arm("all_active_quality", source_rows, source_rows, [], {rule: True for rules in QUALITY_RULE_ARMS.values() for rule in rules}, minimum_residual_chars, token_counter, False),
    }
    plan = build_plan(
        source_rows,
    ...
    arms["web_control_span_candidate"] = _arm("web_control_span_candidate", source_rows, candidate["records"], candidate["transformations"], {}, minimum_residual_chars, token_counter, False)
    cumulative_rules = {
        rule: True
...
def _write_report(input_path: Path, output_path: Path, minimum_residual_chars: int) -> JsonMap:
    report = run_quality_matrix(_read_jsonl(input_path), minimum_residual_chars=minimum_residual_chars, token_counter=lambda text: len(text.split()))
...
def main() -> int:
    ...
    report = _write_report(args.input, args.output, args.minimum_residual_chars)
