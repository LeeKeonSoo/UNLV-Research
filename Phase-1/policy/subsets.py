...
def _axis_scores(record: Dict[str, Any]) -> Dict[str, float]:
    ...
    quality = float(metrics["reference_quality_score"]["score"])
    ...
    return {
        ...
        "diagnostic_predictive_utility": float((diagnostic.get("predictive_utility_proxy") or {}).get("score") or 0.0),
    }
...
def _objective_components(record: Dict[str, Any]) -> Dict[str, float]:
    ...
    quality = float(axes["quality"])
    if quality < 0.65:
        quality_learnability_support = 0.0
    elif quality < 0.84:
        quality_learnability_support = (quality - 0.65) / 0.19
    elif quality <= 0.94:
        quality_learnability_support = 1.0
    else:
        quality_learnability_support = max(0.35, 1.0 - ((quality - 0.94) / 0.08) * 0.65)
    quality_tail_penalty = max(0.0, min(1.0, (quality - 0.97) / 0.03))
    learnability_support = max(
        0.0,
        min(
            1.0,
            (0.35 * quality_learnability_support)
    ...
    return {
        ...
        "quality_learnability_support": float(quality_learnability_support),
        "quality_tail_penalty": float(quality_tail_penalty),
...
def _style_bucket_from_text(text: Any) -> str:
    ...
    markdown_markers = sum(marker in lowered for marker in ("**", "__", "## ", "```", "`"))
    instructional_hits = sum(
        keyword in lowered
        for keyword in (
            "lesson",
            "section",
    ...
    technical_hits = sum(
        token in lowered
        for token in ("{", "}", "()", "[]", "::", "http", "www.", "api", "sql", "function", "class")
    )
    digit_ratio = sum(ch.isdigit() for ch in raw) / max(len(raw), 1)

    if lines and (bullet_lines / max(len(lines), 1) >= 0.25 or markdown_markers >= 2):
        return "structured_list"
    if instructional_hits >= 2:
        return "instructional"
    if technical_hits >= 2 or digit_ratio >= 0.08:
...
def _cluster_backbone_audit(
    ...
    *,
    ...
    cluster_representatives: List[set[str]] = []
    style_purity_scores: List[float] = []
    domain_purity_scores: List[float] = []
    for cluster_id in cluster_ids:
        token_sets = _cluster_text_sets(conn, dataset=dataset, cluster_id=cluster_id, limit=3)
        if len(token_sets) >= 2:
            for i in range(len(token_sets)):
                for j in range(i + 1, len(token_sets)):
                    within_scores.append(_jaccard(token_sets[i], token_sets[j]))
        if token_sets:
            cluster_representatives.append(set.union(*token_sets))
        sample_rows = _cluster_sample_rows(conn, dataset=dataset, cluster_id=cluster_id, limit=3)
        if sample_rows:
    ...
    for i in range(len(cluster_representatives)):
        for j in range(i + 1, len(cluster_representatives)):
            between_scores.append(_jaccard(cluster_representatives[i], cluster_representatives[j]))
    ...
    lexical_separation_pass = bool(separation_margin >= 0.005)
    # KMeans over broad corpora can have weak lexical separation while still acting as
    # a useful coverage backbone when it preserves domain/style anchors.
    anchor_purity_pass = bool(domain_purity_proxy >= 0.85 and style_purity_proxy >= 0.65)
    passed = bool(structural_pass and coherence_pass and (lexical_separation_pass or anchor_purity_pass))
    ...
    if not lexical_separation_pass and not anchor_purity_pass:
        failure_reasons.append("neither_lexical_separation_nor_anchor_purity_passed")
    return {
        "passed": passed,
    ...
    *,
    ...
    for bucket, selected_count in sorted(selected_counts.items()):
        ...
        for record in ordered[:target]:
            ...
            if uid:
                chosen_uids.add(uid)
        bucket_selected[bucket] = int(min(target, len(ordered)))
    ...
    *,
    ...
    for key, selected_count in sorted(selected_counts.items()):
        ...
        for level_key in (exact_key, *fallback_keys):
            ...
            ordered = sorted(
                candidate_levels.get(level_key, []),
                key=lambda record: (
                    _stable_hash_score(str(record.get("chunk_uid") or ""), seed=seed),
                    str(record.get("chunk_uid") or ""),
                ),
...
def _utility_probe_sensitivity_status(dataset: str) -> Dict[str, Any]:
    ...
    probe_valid = bool(sensitivity.get("probe_valid", sensitivity.get("order_pass")))
    return {
        "available": True,
        "probe_valid": probe_valid,
        "status": "valid" if probe_valid else "invalid",
        "selector_tuning_allowed": bool((root_cause or {}).get("selector_tuning_allowed", probe_valid)),
        "source": str(UTILITY_SENSITIVITY_AUDIT_PATH),
...
def generate_subsets(
    ...
    for profile_name, profile in profiles["profiles"].items():
        ...
        dataset_names = sorted(scoring_manifest["datasets"].keys())
        _progress(
            f"profile start: {profile_name} datasets={len(dataset_names)} "
        ...
        for dataset, dataset_meta in scoring_manifest["datasets"].items():
            ...
            style_coverage_support = _distribution_bucket_support(
                _selected_style_counts_from_text_map(selected, text_map),
            ...
            should_compute_ood = bool(stage_c["compute_ood_utility_report"]) and len(dataset_names) > 1
            should_enforce_ood = bool(stage_c["enforce_ood_utility_pass"])
            if should_compute_ood:
                utility_mode = "in_domain_required_ood_report"
                ood_eval_datasets = [str(name) for name in dataset_names if str(name) != str(dataset)]
            ...
            expected_ood_pair_count = max(0, len(dataset_names) - 1) if (should_compute_ood or should_enforce_ood) else 0
            ...
            utility_failure_analysis = {
                ...
                "interpretation": (
                    ...
                    "multi-matched Stage-A is canonical, while Stage-A random remains a distribution-shift stress diagnostic."
                ),
            }
