...
from quality_rule_evidence import (
    CANDIDATE_QUALITY_RULE_KEYS,
    EMPTY_HTML_SHELL_REASON,
    ERROR_NAVIGATION_ONLY_REASON,
    ...
    POLICY_IDS,
    POLICY_VERSIONS,
    URL_DIRECTORY_ONLY_REASON,
    WEB_CHROME_ONLY_REASON,
    is_empty_html_shell,
    is_error_navigation_only,
    is_license_comment_only,
    is_url_directory_only,
    is_web_chrome_only,
)
...
def evaluate_quality_retention(
    ...
    for row in materialized:
        ...
        if record_id in generated_record_observations:
            ...
            "explicit_error_navigation_only_chunk_candidate" in enabled
            and is_error_navigation_only(text)
        ):
            decision = _decision(
                row=row,
                decision=QUALITY_REJECT,
                evaluated_policy_ids=evaluated_policy_ids,
                policy_id=POLICY_IDS["explicit_error_navigation_only_chunk_candidate"],
                reason_code=ERROR_NAVIGATION_ONLY_REASON,
                trigger="explicit_error_marker_plus_three_closed_navigation_lines_only",
                non_trigger_boundary="error_explanations_code_and_non_navigation_payload_are_retained",
                evidence="all_nonblank_lines_are_closed_error_or_navigation_markers",
            )
        elif "url_directory_only_chunk_candidate" in enabled and is_url_directory_only(text):
            decision = _decision(
                row=row,
                decision=QUALITY_REJECT,
                evaluated_policy_ids=evaluated_policy_ids,
                policy_id=POLICY_IDS["url_directory_only_chunk_candidate"],
                reason_code=URL_DIRECTORY_ONLY_REASON,
                trigger="five_or_more_url_only_lines_and_no_other_payload",
                non_trigger_boundary="annotated_reference_lists_and_mixed_payload_are_retained",
                evidence="all_nonblank_lines_are_standalone_urls",
            )
        else:
            decision = _decision(
                row=row,
    ...
    return decisions, {
        ...
        "candidate_policy_used": bool(CANDIDATE_QUALITY_RULE_KEYS.intersection(enabled)),
